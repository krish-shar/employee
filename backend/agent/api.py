...
async def get_agent_run_with_access_check(client, agent_run_id: str, user_id: str):
    ...
    logger.info(f"Getting agent run with access check for {agent_run_id}")
...
async def get_or_create_project_sandbox(client, project_id: str):
    ...
    logger.info(f"project_data: {project_data}")
    ...
    logger.info(f"sandbox_pass: {sandbox_pass}")
...
async def start_agent(
    ...
    client = await db.client
    await verify_thread_access(client, thread_id, user_id)
    logger.info(f"Verified thread access for thread: {thread_id}")
    ...
    logger.info(f"Project ID: {project_id}, Account ID: {account_id}")
