...
mimetypes.add_type("image/svg", ".svg")
mimetypes.add_type("image/pdf", ".pdf")
...
class SandboxVisionTool(SandboxToolsBase):
        ...
        try:
            ...
            if not mime_type or not mime_type.startswith('image/'):
                ...
                if ext == '.jpg' or ext == '.jpeg': mime_type = 'image/jpeg'
                    ...
                    return self.fail_response(f"Unsupported or unknown image format for file: '{cleaned_path}'. Supported: JPG, PNG, GIF, WEBP, PDF, SVG.")
    ...
    @openapi_schema({
        ...
        "function": {
            "name": "see_pdf",
            "description": "Allows the agent to 'see' a PDF file located in the /workspace directory. Provide the relative path to the PDF file. The PDF content will be made available in the next turn's context.",
            "parameters": {
                "type": "object",
                "properties": {
                    "file_path": {
                        "type": "string",
                        "description": "The relative path to the PDF file within the /workspace directory (e.g., 'docs/report.pdf')."
                    }
                },
                "required": ["file_path"]
            }
    ...
    @xml_schema(
        ...
        example='''
        <!-- Example: Request to see a PDF named 'report.pdf' inside the 'docs' folder -->
        <see-pdf file_path="docs/report.pdf"></see-pdf>
        '''
    )
    async def see_pdf(self, file_path: str) -> ToolResult:
        """Reads a PDF file, converts it to base64, and adds it as a temporary message."""
        try:
            # Ensure sandbox is initialized
            await self._ensure_sandbox()
            ...
            cleaned_path = self.clean_path(file_path)
            full_path = f"{self.workspace_path}/{cleaned_path}"
            logger.info(f"Attempting to see PDF: {full_path} (original: {file_path})")
            
            # Check if file exists and get info
            try:
                file_info = self.sandbox.fs.get_file_info(full_path)
                if file_info.is_dir:
                    return self.fail_response(f"Path '{cleaned_path}' is a directory, not a PDF file.")
                if not file_info.is_file:
                    return self.fail_response(f"File '{cleaned_path}' does not exist.")
                
                # Read PDF file content
                pdf_bytes = self.sandbox.fs.download_file(full_path)
                ...
                # Add the temporary message using the thread_manager callback
                # Use a distinct type like 'pdf_context'
                await self.thread_manager.add_message(
                    thread_id=self.thread_id,
                    type="pdf_context", # Use a specific type for this
                    content=pdf_context_data, # Store the dict directly
                    is_llm_message=False # This is context generated by a tool
                )

                logger.info(f"Added PDF context message for '{cleaned_path}' to thread {self.thread_id}")
                
                # Inform the agent the PDF will be available next turn
                return self.success_response(f"Successfully loaded the PDF '{cleaned_path}'.")
            except Exception as e:
                logger.error(f"Error processing see_pdf for {file_path}: {e}", exc_info=True)
                return self.fail_response(f"An unexpected error occurred while trying to see the PDF: {str(e)}")
